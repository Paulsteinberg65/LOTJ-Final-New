...
public class ObjectMerge : EditorWindow {
	...
	private static int yieldCountMax = 3000;	//Wait this many iterations before trying yield
	private const int hammerTimeCut = 100;
	private static int yieldCountMin = 1000;
	...
	public static bool YieldIfNeeded() {
		if (yieldIterCount++ > yieldCountMax) {
			if (hammerTime && yieldCountMax > yieldCountMin) {
				yieldCountMax -= hammerTimeCut;
			} else {
				yieldCountMax += hammerTimeCut;
			}
