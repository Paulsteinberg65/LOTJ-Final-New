		...
		if (merged) {
			...
			if (renderSettingsFoldout) {
				...
				ObjectMerge.DrawGenericRow(new ObjectMerge.GenericRowArguments {
					...
					left = delegate { mySceneData.fogDensity = EditorGUILayout.FloatField("Linear Density", mySceneData.fogDensity); },
					...
					right = delegate { theirSceneData.fogDensity = EditorGUILayout.FloatField("Linear Density", theirSceneData.fogDensity); },
					drawButtons = true
				});
				//Linear Fog Start
				ObjectMerge.DrawGenericRow(new ObjectMerge.GenericRowArguments {
...
public struct SceneData {
	 ...
	 */
	//Wher is "use in forward rendering?"
